...
### sampleSheetFile 
Path to sample sheet file. Must be a .csv file and if exported from Quanta Soft it must include the following columns: Well, Sample, TargetType and Target. 
The entries in the column 'TargetType' must be either 'Ch1Unknown' or 'Ch2Unknown', and is used to extract rows with information from either channel 1 or channel 2. An example file has been included in the package, which can be found using `system.file("extdata", "Sample_names.csv", package="PoDCall")`.
File exported from QX Manager must include the following columns: Well, Sample Description 1, Dye Name(s) and Target.
...
### dropletVolume
Tne average droplet volume in µL used to calculate concentrations. Default 
dropletVolume=0.000851, can be changed to the updated value 0.000795
...
where neg_drop_tar is number of negative droplets in channel 1 (target), and Vd
is average droplet volume (0.000851 µL by deafult).

### c_ctrl 
Concentration of control, calculated by the formula 
$-\log\dfrac{\dfrac{\text{neg_drop_ctrl}}{\text{tot_droplets}}}{V_d}$ 
where neg_drop_ctrl is number of negative droplets in channel 2 (control), and 
Vd is average droplet volume (0.000851 µL by deafult).
...
ampData <- importAmplitudeData(dataDirectory=dataPath, skipLines=0)
...
podcallMultiplot(plateData=ampData,
                ...
                channel=ch, sampleID=sampleSheet$sample_id, colCh=1)
